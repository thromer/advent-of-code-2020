...
class Rules:
  ...
  def test_message_prefix(self, rule_id, message, depth):
    ...
    if type(rule) == str:
      if message[0] == rule:
        print(f'{depth=} matched {rule} suffix <{message[1:]}>')
        result = True
    ...
    # if i am 42 or 31 then we *also* should try consuming each
    # remaining suffix with ourselves ?
    
    if result:
      print(f'{depth=} matched {rule_id=}')
    return result, result_suffixes
